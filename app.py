...
if DB_CONFIG:
    ...
    # F-string 내에 {}가 많아지면 f-string 안에 f-string을 쓸 수 없으므로,
    # 사용자 이름과 비밀번호를 별도 변수로 분리하여 사용합니다.
...
def calculate_intrinsic_value_per_share(stock_code, base_date, bond_10yr_rate_input, inflation_rate_input):
    ...
    try:
        ...
        UNIT_MULTIPLIER_FS = 100000000 # 억원 -> 원 (kor_fs 데이터 단위)
...
if st.sidebar.button("내재가치 계산 시작"):
    if not DB_CONFIG or not engine:
        ...
        # 1. 전체 종목코드 불러오기 (kor_ticker에서 종목명, 종가만 가져옴)
